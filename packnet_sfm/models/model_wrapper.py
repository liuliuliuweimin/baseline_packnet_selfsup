...
class ModelWrapper(torch.nn.Module):
    ...
    def __init__(self, config, resume=None, logger=None, load_datasets=True):
        ...
        # change lr when use checkpoint, use code below
        # self.resume['optimizer']['param_groups'][0]['lr']=4.5e-05
        # self.resume['optimizer']['param_groups'][1]['lr']=4.5e-05
        # self.config['model']['optimizer']['depth']['lr']=4.5e-05
        # self.config['model']['optimizer']['pose']['lr']=4.5e-05
        ...
        if load_datasets:
            ...
            validation_requirements = {'gt_depth': True, 'gt_pose': True}
...
def setup_dataset(config, mode, requirements, **kwargs):
    ...
    for i in range(len(config.split)):
        ...
        elif config.dataset[i] == 'NuScenes':
            from packnet_sfm.datasets.nuscenes_dataset import NuScenesDataset
